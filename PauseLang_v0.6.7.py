...
- Fixed test_labels and test_aliases (flag behavior)
- Added SETF macro for explicit flag setting
- Enhanced documentation of flag semantics
- Clarified META vs DATA lane behavior
- Better STORE convention documentation

Previous features (v0.6.6):
- Label support in compiler
...
class PauseLangCompiler:
    ...
    MACROS = {
        ...
        'SETF': ['PUSH', 'ADD2'],              # Set flags from TOS (adds 0, preserves value)
    }
...
║  Test suite fixed, enhanced docs & debugging             ║
╚═══════════════════════════════════════════════════════════╝
""")
    print("\nSelect mode:")
    ...
    if choice == '1':
        ...
        try:
            ...
            vm = PauseLangVM(gas_limit=5000, trap_policy=trap_mode, debug=False)
            print("\nExecuting...")
            result = vm.execute(data, pauses, labels=labels)
