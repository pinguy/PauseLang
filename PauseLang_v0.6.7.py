"""
PauseLang v0.6.7 - Patched Version
======================================
Changes in v0.6.7 (Patches Applied):
- Fixed INC/DEC/NOT macros with immediate values and proper stack handling.
- Added JUMP_IF_NONZERO opcode (0.32) for correct JNZ alias semantics.
- Updated compiler to handle macro tuples with immediates.
- Adjusted NOT macro to use SWAP for boolean flip (1 - TOS).
- Doc updates: Warn on SETF with non-zero operand; added JUMP to conventions.
- Minor: No breaking changes; all tests still pass.
...
class PauseLangCompiler:
    ...
    MACROS = {
        ...
        'NOT':       [('PUSH', 1), 'SWAP', 'SUB2'],  # 1 - TOS (boolean flip; note: for >1, gives negative)
...
def main():
    print(f"""
╔═══════════════════════════════════════════════════════════╗
║  PauseLang {SPEC['version']} - Patched for Macros & JNZ     ║
...
""")
    ...
    print("\n✨ PauseLang v0.6.7 patched - Ready for action!")
