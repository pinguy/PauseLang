"""
PauseLang v0.7.5 - Hardened Edition
======================================
Changes in v0.7.5:
- FIXED: `NOT` macro now correctly computes bitwise NOT (~x) instead of 1-x.
- ADDED: `LNOT` macro for logical NOT (1-x, useful for 0/1 boolean flips).
- ADDED: `NEG` macro for arithmetic negation (0-x).
- FIXED: VM now auto-detects and strips sync phrase even when sync=False (prevents stray HALT).
- FIXED: `test_macros_not` now resets VM between subtests (prevents state leakage).
...
class PauseLangCompiler:
    ...
    MACROS = {
        ...
        'NOT':       [('PUSH', -1), 'SWAP', 'SUB2'],  # Bitwise NOT (~x = -1 - x)
        'LNOT':      [('PUSH', 1), 'SWAP', 'SUB2'],   # Logical NOT for 0/1 (1 - x)
        'NEG':       [('PUSH', 0), 'SWAP', 'SUB2'],   # Arithmetic negation (0 - x)
        'SETF':      [('PUSH', 0), 'ADD2'],
    }
...
class TortureTests:
    ...
    def run_all():
        ...
        print("\n🔥 TORTURE TEST SUITE v0.7.5 (HARDENED) 🔥")
        print("=" * 50)
        passed = 0
        failed = 0
        for test in tests:
