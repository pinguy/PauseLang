"""
PauseLang v0.6.6 - Production Enhancements
===========================================
New features:
- Label support in compiler (no more manual PC counting!)
- Instruction aliases for ergonomics
- Enhanced disassembler with labels and state
- Clear documentation of conventions
- Division/modulo semantics specified

Previous fixes from v0.6.5a:
...
"""
Division and Modulo Behavior (Python-compatible):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DIV2: Truncates toward zero (matches spec)
  • 7 ÷ 2 = 3      • -7 ÷ 2 = -3
  • 7 ÷ -2 = -3    • -7 ÷ -2 = 3
  
MOD2: Result sign follows divisor (Python %)  
  • 7 % 3 = 1      • -7 % 3 = 2
  • 7 % -3 = -2    • -7 % -3 = -1

For C-style remainder: sign(a) * abs(a % b)
"""
...
║                                                              ║
║ FLAG UPDATES:                                                ║
║ • Most ops update ZERO, ODD, NEGATIVE flags                  ║
║ • PASS explicitly does NOT update flags                      ║
║ • Control ops need prior flag-setting operation              ║
║                                                              ║
║ STORE CONVENTION:                                            ║
...
class PauseLangVM:
    ...
    def disassemble(self, show_labels: bool = True, show_state: bool = False, 
        ...
        """Enhanced disassembler with label support"""
        ...
        for step in self.execution_trace:
            ...
            # Format line
            if compact:
                lines.append(f"{pc:04d}: {step['opcode']:12} {step['value']:6d}")
            else:
                result_str = str(step['result'])[:20]
                lines.append(
                    f"{label_col}{pc:04d}: {cat_icon} {step['opcode']:12} "
                    f"{step['value']:6d} → {result_str:20}{state_col}"
                )
...
class PauseLangCompiler:
    ...
    MACROS = {
        'INC': ['PUSH', 'PUSH', 'ADD2'],      # Increment TOS
        'DEC': ['PUSH', 'PUSH', 'SUB2'],      # Decrement TOS
        ...
        'NOT': ['PUSH', 'PUSH', 'SUB2'],      # Logical NOT (0→1, n→0)
    }
            ...
            if opcode in PauseLangCompiler.MACROS:
                ...
                for macro_op in PauseLangCompiler.MACROS[opcode]:
                    if macro_op in OPCODE_TO_PAUSE:
                        ...
                        data.append(value)
                        comments.append(f"{macro_op} (from {original_opcode})")
            elif opcode in OPCODE_TO_PAUSE:
...
def demo_instruction_reference():
    ...
    for macro, expansion in PauseLangCompiler.MACROS.items():
        print(f"  {macro:10} → {' '.join(expansion)}")
...
def main():
    print(f"""
╔═══════════════════════════════════════════════════════════╗
║  PauseLang {SPEC['version']} - Production-Ready Temporal VM  ║
║  Now with labels, aliases, and enhanced debugging!       ║
╚═══════════════════════════════════════════════════════════╝
""")
    print("\nSelect mode:")
    ...
    print("\n✨ PauseLang v0.6.6 ready - Production grade!")
